describe MasterLoader do
  ...
  it "caches values for each key" do
    ...
    data_loader = MasterLoader.new do |ids|
      calls += 1
      ids.map {|id| id}
    end
  ...
  it 'can be passed custom cache' do
    ...
    data_loader = MasterLoader.new(cache: Cache.new) do |ids|
      ids.map {|id| id}
    end
  ...
  it 'can disable the cache' do
    data_loader = MasterLoader.new(cache: nil) do |ids|
      ids.map {|id| ids}
    end
  ...
  it 'accepts an interceptor chain' do
    ...
    loader = MasterLoader.new(interceptor: interceptor) do |ids|
      ids.map {|id| ids}
    end
  ...
  it "can have nested loaders" do
    ...
    data_loader2 = MasterLoader.new do |ids|
      ids.map {|id| id * -1}
    end

    data_transformer = MasterLoader.new do |ids|
  ...
  it "can zip nested loaders" do
    ...
    parent_loader = MasterLoader.new do |ids|
